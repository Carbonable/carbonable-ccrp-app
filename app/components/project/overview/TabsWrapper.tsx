                ...
                <Tab.List className="flex overflow-x-scroll">
                    {tabs.map((tab, index) => (
                        <Tab key={`tab_${index}`} className={({ selected }) => `
                            ${selected ? 'text-neutral-100 border-b border-neutral-100' : 'px-0 text-neutral-300'}
                            font-inter font-normal px-2 py-2 mr-3 text-sm md:mr-2 md:text-base md:px-4 hover:text-neutral-100 outline-none flex flex-nowrap items-center whitespace-nowrap
                        `}>
                            {tab.title}
                        </Tab>
                    ))}
